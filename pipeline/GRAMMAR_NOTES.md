...
**Grammar cards now exist in the app** — 38 of them, one per pattern, in
`Book1A::<Unit>::Grammar` sections. This file remains the human-readable record
of what each unit introduces; the card data is generated by
`add_grammar_cards.py` from the same transcriptions.
...
## The grammar-card shape, as built

`{..., "type": "grammar", "examples": [{ko, en, ja, audio}]}` — see
pipeline/README.md. One pattern per card, 2–4 examples, no clip on the card
itself. 37 of the 38 have examples; 숫자① (Sino-Korean numbers) has none,
because the book teaches it with bare numerals (10원, 34번, 2월 14일) rather
than sentences, and inventing examples seemed worse than shipping none.
